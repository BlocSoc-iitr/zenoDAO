...
def initialize_agent():
    ...
    if os.path.exists(wallet_data_file):
        try:
            with open(wallet_data_file, 'r') as f:
                ...
                # Convert the dictionary to WalletData object
                wallet_data = WalletData.from_dict(wallet_dict)
    ...
    wallet_data_str = agentkit.export_wallet()  # This returns a string
    try:
        # Parse the string into a dictionary
        wallet_dict = json.loads(wallet_data_str)
    except json.JSONDecodeError:
        # If it's not JSON, store it as is
        wallet_dict = {"wallet_data": wallet_data_str}
